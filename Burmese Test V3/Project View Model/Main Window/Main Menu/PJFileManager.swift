...
class PJFileManager : FileManager {
    ...
    func convertToPlist(_ arrayToConvert: [LessonEntry])->[Dictionary<NSString, NSString>]
    {
        ...
        if arrayToConvert.count > 0 {
            ...
            for item : LessonEntry in arrayToConvert {
                var convertDict = Dictionary<NSString,NSString>()
                let fieldValues = [item.burmese,item.roman,item.english,item.insertion,item.lesson,item.lessonEntryIndex,item.categoryindex,item.category,item.lessonEntryCategory,"\(item.istitle)"]
