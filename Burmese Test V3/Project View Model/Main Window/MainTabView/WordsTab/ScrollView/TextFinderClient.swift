...
extension TextFinderClient  : TextFinderIndexing {
    ...
    func setWordAtRowCol(_ word: String, _ row: Int, col: Int)
    {
        if let dataSource = tableView.dataSource as? TableViewDataSource {
            ...
            switch col {
            case 0: // burmese
                dataSource.lessonEntries[row].burmese = word
            case 1: // english
                dataSource.lessonEntries[row].english = word
            case 2: // roman
                dataSource.lessonEntries[row].roman = word
            case 3: // lesson
                dataSource.lessonEntries[row].lesson = word
            case 4: // category
                dataSource.lessonEntries[row].category = word
            default:
                break
            }
        }
    ...
    func recordAtRowCol(_ row: Int)->LessonEntry {
        if let dataSource = tableView.dataSource as? TableViewDataSource {
            return dataSource.lessonEntries[row]
        }
        return LessonEntry()
    }
    ...
    func wordAtRowCol(_ row: Int, col: Int)->NSString
    {
        if let dataSource = tableView.dataSource as? TableViewDataSource {
            ...
            {
            case 0: // burmese
                let word = dataSource.lessonEntries[row].burmese
                if let word = word as NSString? {
                    return word
                }
            case 1: // english
                return wordOrDiacriticWord(dataSource.lessonEntries[row].english, ignoreDiacritic: ignoreDiacritic)
            case 2: // roman
                return wordOrDiacriticWord(dataSource.lessonEntries[row].roman, ignoreDiacritic: ignoreDiacritic)
            case 3: // lesson
                return wordOrDiacriticWord(dataSource.lessonEntries[row].lesson, ignoreDiacritic: ignoreDiacritic)
            case 4: // category
                return wordOrDiacriticWord(dataSource.lessonEntries[row].category, ignoreDiacritic: ignoreDiacritic)
            default:
                break
            }
        }
...
extension TextFinderClient: TextFinderMenuFilterResults {
    ...
    func copyFilteredItems(_ filteredItems :[LessonEntry]) {
        if let dataSource = tableView.dataSource as? TableViewDataSource {
            dataSource.dataSourceViewModel.lessonEntries = filteredItems
        }
    }
    
    func copyUnfilteredItems() {
        if let dataSource = tableView.dataSource as? TableViewDataSource {
            dataSource.dataSourceViewModel.unfilteredLessonEntries = dataSource.lessonEntries
        }
    }
    
    func updateChangedRows() {
        if let dataSource = tableView.dataSource as? TableViewDataSource {
            if let unfilteredWords = dataSource.dataSourceViewModel.unfilteredLessonEntries {
                for word in dataSource.lessonEntries {
                    if word.filtertype == .change {
                        if let filterIndex = word.filterindex {
                            ...
                            word.filtertype = nil
                            dataSource.dataSourceViewModel.unfilteredLessonEntries?[filterIndex] = word
                        }
                    }
                }
            }
    ...
    func deleteMarkedRows() {
        if let dataSource = tableView.dataSource as? TableViewDataSource {
            for rowIndex in dataSource.filterRowsToDelete.reversed() {
                dataSource.dataSourceViewModel.unfilteredLessonEntries?.remove(at: rowIndex)
            }
    ...
    func insertNewRows() {
        if let dataSource = tableView.dataSource as? TableViewDataSource {
            for word in dataSource.lessonEntries.reversed() {
                if word.filtertype == .add {
                    if let wordCopy = word.copy() as? LessonEntry {
                        wordCopy.filtertype = nil
                        if let insertRow = word.filterindex {
                            dataSource.dataSourceViewModel.unfilteredLessonEntries?.insert(wordCopy, at: insertRow + 1)
                        }
                    }
                }
            }
    ...
    func resetToUnfiltered() {
        if let dataSource = tableView.dataSource as? TableViewDataSource {
            ...
            self.insertNewRows()
            if let unfilteredWords = dataSource.dataSourceViewModel.unfilteredLessonEntries {
                dataSource.dataSourceViewModel.lessonEntries = unfilteredWords
            }
        }
    ...
    @objc func filterItemsMenuItem(_ sender: NSMenuItem) {
        ...
        case .off:
            ...
            if let dataSource = tableView.dataSource as? TableViewDataSource {
                dataSource.dataSourceViewModel.unfilteredLessonEntries?.removeAll()
            }
            copyUnfilteredItems()
    ...
    */
    ...
    func checkForMatchInWord(_ word: Any?, matchInString: String?, textToMatch: String, row: Int)->LessonEntry? {
        if  let word = word as? LessonEntry,
